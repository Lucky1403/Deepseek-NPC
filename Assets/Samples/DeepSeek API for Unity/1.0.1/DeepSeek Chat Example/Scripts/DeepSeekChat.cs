...
namespace YagizEraslan.DeepSeek.Unity
{
    public class DeepSeekChat : MonoBehaviour
    {
        ...
        // Use string model name for custom model like deepseek-r1:free
        [SerializeField] private string modelName = "deepseek/deepseek-r1:free";
        [SerializeField] private bool useStreaming = false;
        ...
        private void SendMessage()
        {
            ...
            if (!controllerInitialized)
            {
                controller = new DeepSeekChatController(
                  new DeepSeekApi(config),
                  modelName,  // Use string model here
                            AddFullMessageToUI,
                  AppendStreamingCharacter,
                  useStreaming
                );
